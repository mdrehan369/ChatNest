...
function ChatBox({ groupId, unmount }: any) {
    ...
    useEffect((): any => {
        ; (async () => {
            try {
                ...
                const response = await axios.get(`/api/v1/groupChats?groupId=${groupId}`)
                setChats(response.data.data)
                // setUser(response.data.data.user)
                // roomId.current = response.data.data.roomId
                setSocket(socketClient)
                // const wallpaper = await axios.get(`/api/v1/preferences/wallpaper?user=${groupId}`)
    ...
    //     try {
    //         await axios.delete(`/api/v1/friends?acceptor=${groupId}`)
    //         router.replace("/")
    //     } catch (err) {
    //         console.log(err)
    //     }
    ...
    //     try {
    //         await axios.delete(`/api/v1/chats?user=${groupId}`)
    //         setNewChats([])
    //         setChats([])
    //         toast({
    ...
    //         })
    //         await axios.delete(`/api/v1/preferences/wallpaper?user=${groupId}`)
    //         setWallpaper("")
    //         toast({
    ...
    const handleProfileView = () => {
        router.push(`/profile?groupId=${groupId}`)
    }
        ...
        !loader ?
            <div className="w-[77vw] h-[90vh] rounded border-[0px] border-gray-200" id="box">
                <div className="w-full h-[10%] flex items-center justify-between px-8 py-4 bg-gray-300 shadow-sm border-b-[1px] border-gray-400 relative">
                    <div className="flex items-center justify-start gap-3" onClick={() => router.push(`/profile?groupId=${groupId}`)}>
                        <Avatar>
                ...
                >
                    <div className="w-full flex flex-col items-start justify-end p-3 gap-1 relative z-0 min-h-full" id="inner">
                        {chats.concat(newChats).map((chat: any, index) => <div key={index} className={` w-fit flex items-center justify-start ${chat.from === groupId ? ' flex-row self-start' : ' flex-row-reverse self-end'} gap-2 px-2 `}>
                            <Avatar>
                                <CldImage src={chat.to === groupId ? (loggedUser?.profile_pic || "LawKeeper/ghb4flnfqwgk3fyd6zv2") : (user?.profile_pic || "LawKeeper/ghb4flnfqwgk3fyd6zv2")} width={100} height={100} alt="" className=" object-cover" />
                                <AvatarFallback>CN</AvatarFallback>
                            </Avatar>
                            {
                                chat.from === groupId ?
    ...
    useEffect(() => {
        ...
        socket.on("globalMsgReceived", (message: IChat) => {
            ...
            if (message.to.toString() === user?._id.toString() || message.from.toString() === user?._id.toString()) {
                setGroups((prev: any) => prev.map((friend: any) => {
                    if (friend._id === message.from || friend._id === message.to) {
                        if (groupIdRef.current === friend._id) {
                            message.seen = true
                        } else {
                            message.seen = false
                        }
        ...
        socket.on("markOffline", (groupId) => {
            console.log(groupId)
            setGroups((prev: any) => prev.map((friend: any) => {
                if (friend._id === groupId) friend.isOnline = false
                return friend
            }))
        })
        socket.on("markOnline", (groupId) => {
            console.log(groupId)
            setGroups((prev: any) => prev.map((friend: any) => {
                if (friend._id === groupId) friend.isOnline = true
                return friend
            }))
        })
        ...
        !loader && user ?
            <main className="w-[100vw] h-[93vh] flex items-center justify-between p-4 bg-white">
                <div className="w-[20vw] h-full bg-gray-100 p-3 rounded border-[0px] border-gray-400 flex flex-col items-center justify-start gap-2">
                    ...
                    {
                        groups?.map((group: any, index) => <div onClick={() => openChats(group._id)} key={index} className="w-full bg-gray-200 cursor-pointer hover:bg-gray-300 transition-colors duration-500 border-[2px] border-gray-200 rounded p-2 flex items-center justify-start gap-4 pl-4">
                            <Avatar>
                                <CldImage src={group.profile_pic || "LawKeeper/ghb4flnfqwgk3fyd6zv2"} width={100} height={100} alt="" className=" object-cover" />
                                <AvatarFallback>CN</AvatarFallback>
                            </Avatar>
                            <div className="w-full relative flex flex-col items-start justify-center gap-0">
                                ...
                                {/* <span className="text-xl font-semibold text-slate-800">{group.name}</span> */}
                                <div className="w-full flex items-center justify-between text-sm text-gray-600 font-medium italic">
                                    {
                                        group.lastChat &&
                                        <>
                                            <span className={!group.lastChat.seen && group.lastChat.to === user._id ? 'text-green-600' : ''}>{group.lastChat.from === group._id ? group.lastChat.content : `You: ${group.lastChat.content}`}</span>
                                            <span>{ISOtoTime(group.lastChat.createdAt)}</span>
                                        </>
                                    }
                                </div>
                            </div>
                ...
                {
                    groupId != "" ?
                        <ChatBox groupId={groupId} unmount={unmount} key={groupId} />
                        : <NoChat />
                }
            </main>
            : <Loading />
